...
document.getElementById('calculateBtn').addEventListener('click', function() {
    ...
    // สลับลำดับช่อง: จี๋ใหญ่ -> จี๋เล็ก -> เห็ดบาน
    const container = document.getElementById('container');
    const groupLarge = document.getElementById('group-large');
    const groupSmall = document.getElementById('group-small');
    const groupBloom = document.getElementById('group-bloom');

    // นำแต่ละกลุ่มออกก่อนแล้วใส่ลำดับใหม่
    container.insertBefore(groupLarge, container.children[1]);
    container.insertBefore(groupSmall, container.children[2]);
    container.insertBefore(groupBloom, container.children[3]);
});
</script>
</body>
</html>
